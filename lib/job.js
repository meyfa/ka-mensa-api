...
/**
 * Use the handicap source to fetch plans for all canteens, for a few days into
 * the future as defined in the config.
 *
 * @returns {Promise} Resolves to the fetched plan set.
 */
async function fetchFromHandicap () {
  const dayCount = Math.max(config.fetchJob.handicapOptions.futureDays, 0)
...
async function run (cache, logger) {
  ...
  let plans
  if (source === 'jsonapi') {
    plans = await fetchFromJsonApi()
  } else if (source === 'handicap') {
    plans = await fetchFromHandicap()
  } else {
    logger.log('error', 'invalid source setting')
    return
  }
