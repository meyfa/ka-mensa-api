...
/**
 * Read a list of the available plan files, in the form of key strings.
 *
 * @returns {Promise} Resolves to a list of file keys.
 */
async function readCacheDirectory () {
  try {
    const files = await fsPromises.readdir(config.cache.directory)
    ...
    // ignore if directory does not exist, otherwise re-throw
    if (e.code !== ENOENT) {
      throw e
    }
  }
 ...
 */
async function list () {
  const keys = await readCacheDirectory()
  return keys.filter(parseDate)
}
